...
if (typeof window !== "undefined") {
  // potraceFromImageData: 直接从 ImageData 的 Uint8ClampedArray 生成 SVG
  // 参数: data (Uint8ClampedArray), width (number), height (number)
  // 返回: Promise<string> SVG 字符串
  window.potraceFromImageData = async function(data, width, height) {
    await ready();
    var start = wrapStart();
    return start(data, width, height);
  };

  // potraceFromCanvas: 从 canvas 元素生成 SVG
  window.potraceFromCanvas = loadFromCanvas;

  // potraceReady: WASM 就绪 Promise
  window.potraceReady = ready();
}
